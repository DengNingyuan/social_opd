...
from inference import geometric_mean_confidence
...
def test_teacher_formality_prompt_adds_three_angle_instruction() -> None:
    ...
    assert '{"score": <integer 0-100>}' in teacher
    assert "Also report confidence" not in teacher
...
def test_parse_completion_reads_json_score_only() -> None:
    parsed = parse_completion('{"score": 73}')

    assert parsed == {"score": 73.0}


def test_geometric_mean_confidence_uses_token_logprob_mean() -> None:
    assert geometric_mean_confidence([0.0, -2.0]) == pytest.approx(0.367879, abs=1e-6)
