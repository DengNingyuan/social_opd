...
多了三个维度的考虑说明，输出仍然只有 score 和 confidence。
...
在给出最终 score/confidence 前，先从 disagreement-aware 的三个来源展开分析：
data factors、task factors、annotator factors。最终仍只输出 score 和 confidence JSON。
teacher 仍然只输出 score 和 confidence，不输出三个角度。
