...
from data import clip_confidence, clip_score
    ...
    if match:
        try:
            ...
            return {
                ...
                "confidence": clip_confidence(float(payload.get("confidence", 0.5))),
            }
        except (TypeError, ValueError, json.JSONDecodeError):
            pass
    number = re.search(r"-?\d+(?:\.\d+)?", text)
    return {"score": clip_score(float(number.group(0))) if number else 50.0, "confidence": 0.5}
